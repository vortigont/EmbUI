...
static const char T_EN_MQTT_Cmt[] PROGMEM = "MQTT topic prefix could be set to custom value. '$id' macro will be replaced with device ID, i.e. 'mydev/$id/'. Set empty string to reset to default prefix.";
static const char T_EN_MQTT_Host[] PROGMEM = "MQTT server";
static const char T_EN_MQTT_Interval[] PROGMEM = "MQTT update interval, sec.";
static const char T_EN_MQTT_Port[] PROGMEM = "MQTT port";
static const char T_EN_MQTT_Prefix[] PROGMEM = "Override MQTT topic prefix";
...
static const char T_EN_WiFiClientOpts[] PROGMEM = "Connect to WiFi network";
...
static const char T_RU_MSG_APOnly[] PROGMEM = "В режиме AP-only контроллер всегда работает как точка доступа и не будет пытаться подключиться к другим WiFi-сетям";
static const char T_RU_MSG_APProtect[] PROGMEM = "Защитить AP паролем (мин 8 символов)";
static const char T_RU_MSG_DATETIME[] PROGMEM = "Произвольные дата/время в формате ISO YYYY-MM-DDThh:mm:ss (опционально)";
static const char T_RU_MSG_TZONE[] PROGMEM = "Часовая зона";
static const char T_RU_MSG_TZSet01[] PROGMEM = "Установки часовой зоны. Правила смены поясного/сезонного времени применяются автоматически, ручной коррекции не требуется. Если вашей зоны нет в списке, можно выбрать общую зону смещения от Гринвича";
static const char T_RU_NTP_Secondary[] PROGMEM = "пользовательский NTP-сервер (опционально)";
